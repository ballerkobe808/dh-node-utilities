 ...
 * @returns {Error}
 */
function CustomError(errorType, messageType, message, httpStatus, statusCode, err, errorCode, ignoreLogging) {
  ...
  // check if we want to ignore this error when logging.
  if (ignoreLogging) {
    error.ignoreLogging = true;
  }
 ...
 */
exports.createError = function (errorType, messageType, message, httpStatus, statusCode, err, errorCode, ignoreLogging) {
  return new CustomError(errorType, messageType, message, httpStatus, statusCode, err, errorCode, ignoreLogging);
};

/**
 * Creates a shortened error object.
 ...
 * @returns {CustomError}
 */
exports.createShortError = function (httpStatusCode, status, message, err) {
  return new CustomError(null, null, message, httpStatusCode, status, err);
};
