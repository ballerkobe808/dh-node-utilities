 ...
 */
exports.contains = (string, match, caseSensitive) => {
  if (string && match) {
 ...
 * @returns {*}
 */
exports.prettifyJSONObject = (obj) => {
  let result = '';
 ...
 * @returns {*}
 */
exports.commaSeparatedNumber = (val) => {
  val = val.toString();
 ...
 * @returns {*}
 */
exports.commaSeparatedCurrency = (val) => {
  val = val.toString();
  let result = '$';
  while (/(\d+)(\d{3})/.test(val.toString())) {
    val = val.toString().replace(/(\d+)(\d{3})/, '$1'+','+'$2');
 ...
 * @returns {boolean}
 */
exports.existsAtPosition = (value, searchString, position) => {
 ...
 */
exports.countOfCharacter = (value, match) => {
  try {
    let count = 0;
...
/**
 * Splits a semi colon delimited list of string into an array.
 * @param value - The value.
 */
exports.splitSemicolonString = (value) => {
  // is the value is empty return null.
  if (exports.isEmpty(value)) {
    return null;
  }

  try {
    // split the string on the semi colon.
    let values = value.split(';');
    let result = [];

    // trim all the values.
    for (let i = 0; i < values.length; i++) {
      if (!exports.isEmpty(values[i].trim())) {
